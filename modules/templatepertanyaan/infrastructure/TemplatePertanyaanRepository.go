...
func (r *TemplatePertanyaanRepository) GetDefaultWithAnswareByUuid(
	...
	err := r.db.Debug().
		...
		Joins("LEFT JOIN m_fakultas af ON a.fakultas = af.kode_fakultas").
		Joins("LEFT JOIN m_program_studi ap ON a.prodi = ap.kode_prodi").
		Joins("LEFT JOIN m_fakultas uf ON u.fakultas = uf.kode_fakultas").
		Joins("LEFT JOIN m_program_studi up ON u.prodi = up.kode_prodi").
		Select(`
			a.id as ID,
			a.uuid as UUID,
			a.id_bank_soal as IdBankSoal,
			...
			a.status as Status,
			CASE
				WHEN u.prodi IS NOT NULL AND u.prodi != '' THEN CONCAT("PRODI ", COALESCE(up.nama_prodi, ap.nama_prodi, u.prodi))
				WHEN u.fakultas IS NOT NULL AND u.fakultas != '' THEN CONCAT("FAKULTAS ", COALESCE(uf.nama_fakultas, af.nama_fakultas, u.fakultas))
			...
			COALESCE(NULLIF(TRIM(a.fakultas), ''), uf.nama_fakultas, af.nama_fakultas, u.fakultas) as Fakultas,
			COALESCE(NULLIF(TRIM(a.prodi), ''), up.nama_prodi, ap.nama_prodi, u.prodi) as Prodi,
			a.unit as Unit,
